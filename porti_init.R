...
# com <- st_read("~/R/porti/Limiti01012021/Com01012021/Com01012021_WGS84.shp") %>% filter(PRO_COM == 55032)
...
terni_utm32 <- st_transform(terni_fltr, 32632) 
...
dists <- c(025, 050, 075, 100, 200)

getBufferInt <- function(dist, code) {
  print( paste(dist, code,  sep = "--"))
  name <- str_pad(dist, 3, pad = "0")
...
v <- vect("punti_misura.shp")

r <- rast("/home/rmorelli/R/porti/data/impermeabilizzazione_utm32.tif")
...
map(codes_2018, function(c) {
  ...
  dfs <-  lapply(fls,
                 function(x) {
                   read_csv(x,
                            col_types = cols(ID = col_skip(), `v1$Site` = col_skip()))
                 })
  do.call(cbind, dfs) %>%
    cbind(pt_misura$Site) %>%
    setNames(c(dists, "site")) %>%
