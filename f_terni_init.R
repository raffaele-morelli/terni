...

{
  ...
  dominio <- st_read("~/R/terni/data/dominio/dominio_100m.shp")
...
getBufferRastImp <- function(dist) {
  ...
  g <- ggplot() + 
    ...
    geom_sf(data = st_as_sf(v1), color = "red", fill = "yellow")
  ggsave(g, filename = glue("out/imperviousness/imp_{name}.jpg"))
  
  return(extract(imperm, v1, xy = TRUE))
}

# i impermeabilizzazione all'interno dei buffer
walk(c(200), ~ getBufferRastImp(.x))
